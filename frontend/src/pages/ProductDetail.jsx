...
import { Layout, PageHeader, Loader, Lightbox } from "../components/UI";
...
export default function ProductDetail() {
  ...
  const infoRows = [
    ...
    {
      label: "Total Scans",
      value: String(product.scanCount || 0),
      icon: ScanLine,
    },
    {
      label: "First Scanned",
      value: product.firstScannedAt
      ...
      <PageHeader
        ...
        action={
          <button
            ...
            className="btn-secondary text-xs sm:text-sm px-3 sm:px-5"
          >
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4 lg:space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4 lg:hidden">
            <QRPanel
              title="Unit QR"
              qrCode={product.unitQrCode}
              productId={product._id}
              type="unit"
            />
            <QRPanel
              title="Carton QR"
              qrCode={product.cartonQrCode}
              productId={product._id}
              type="carton"
            />
          </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {product.images.map((img, i) => (
                  <img
                    ...
                    className="w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 rounded-xl object-cover ring-1 ring-gray-100 cursor-zoom-in hover:ring-primary-300 hover:scale-105 transition-all duration-200"
                  />
                ))}
              </div>
            </div>
                ...
                ({ label, value, icon: Icon }) =>
                  value && (
                    ...
                    >
                      ...
                      <span className="text-sm font-semibold text-gray-800 capitalize text-right">
                        {value}
                      </span>
                    </div>
                  ),
          ...
          <div className="card p-4 lg:p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 bg-primary-50 px-3 py-2 rounded-xl">
                ...
                <span className="text-xs text-primary-500 hidden sm:inline">
                  total scans
                </span>
              </div>
            </div>
            {product.firstScannedAt && (
              <p className="text-xs text-gray-400 mt-3 font-body">
                First scanned{" "}
                {new Date(product.firstScannedAt).toLocaleDateString("en-GB", {
                  weekday: "long",
                  day: "numeric",
                  month: "long",
                  year: "numeric",
        ...
        <div className="hidden lg:flex lg:col-span-1 flex-col gap-4">
          ...
          <div className="card bg-primary-50 border border-primary-100">
            <p className="text-xs text-primary-700 leading-relaxed font-body">
              <strong>Unit QR</strong> → individual packs for consumers.
              <br />
              <br />
